...
const log = consola.withTag('gemini-search-mcp');

export interface CreateGeminiSearchMcpServerOptions {
	name?: string;
	version?: string;
	apiKey?: string;
...
export function createGeminiSearchMcpServer(options: CreateGeminiSearchMcpServerOptions = {}): McpServerInstance {
	const { name = 'gemini-web-search', version = '1.0.0' } = options;
