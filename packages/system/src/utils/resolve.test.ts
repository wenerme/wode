...
test('resolve legacy', () => {
	const base = { name: '@org/nice', main: 'main.cjs', module: 'module.mjs', system: 'system.js' };
	assert.equal(legacy(base, { fields: ['system'] }), './system.js');
	expect(legacy({ main: 'index.js' }, { fields: ['system'] }), './system.js').toBeFalsy();
});
	...
	const base = {
		...
		exports: {
			'.': {
				...
				system: { production: './dist/system.prod.js', default: './dist/system.js' },
			},
			'./assets/*': './dist/assets/*',
			'./src/*': './src/*.ts',
		},
