...
export interface ConvertOptions {
	...
	/**
	 * Pre-resolved context data for enriching block rendering.
	 * Populated externally (e.g. via API calls) before calling the converter.
	 * Keys are entity IDs (task_id, user_id), values are display info.
	 */
	entityInfo?: Record<string, EntityInfo>;
}

export interface EntityInfo {
	/** Display label (task summary, user name, etc.) */
	label?: string;
	/** Link URL */
	href?: string;
	/** Extra metadata (e.g. completed, due date) */
	[key: string]: unknown;
}
 ...
 */
export function buildMarkdownFromDocBlock(blocks: FeishuBlock[], options?: ConvertOptions): string {
	...
	if (opts.includeTitle) {
		pageTitleText = renderElements(getBlockTextData(pageBlock)?.elements, opts).trim();
		if (pageTitleText) {
			if (opts.includeBlockId) {
				lines.push(`<!-- block:${pageBlock.block_id} -->`);
			}
			lines.push(`# ${pageTitleText}`);
			lines.push('');
		}
	}
	...
	for (let i = 0; i < children.length; i++) {
		const child = blockMap.get(children[i]);
		if (!child) continue;

		// Skip first heading if it duplicates the page title
		if (i === 0 && pageTitleText && child.block_type === BlockType.Heading1) {
			const h1Text = renderElements(getBlockTextData(child)?.elements, opts).trim();
			if (h1Text === pageTitleText) continue;
		}
		...
		const rendered = renderBlock(child, blockMap, opts, 0, orderedCounter);
		if (rendered) {
			lines.push(rendered);
		}
	}
	...
	return (
		lines
			.join('\n')
			.replace(/\n{3,}/g, '\n\n')
			// Collapse blank lines between consecutive list items (but not before first item)
			.replace(/(^[ \t]*(?:[-*]|\d+\.|- \[[ x]\]) .+)\n\n(?=[ \t]*(?:[-*]|\d+\.|- \[[ x]\]) )/gm, '$1\n')
			.trim() + '\n'
	);
}
...
export interface FeishuBlock {
	...
	task?: { task_id?: string };
	add_ons?: { component_id?: string; component_type_id?: string; record?: string };
	[key: string]: unknown;
}
...
function renderElements(elements: FeishuTextElement[] | undefined, opts: Required<ConvertOptions>): string {
	...
	for (const el of elements) {
		if (el.text_run) {
			...
			// Strikethrough wraps around everything including code
			...
			const userInfo = opts.entityInfo[`user:${userId}`];
			// Format: [@name] or [@user_id] — detectable on push for round-trip
			parts.push(`[@${userInfo?.label || userId}]`);
		} else if (el.equation) {
...
function renderBlock(
	...
	if (type === BlockType.Ordered) {
		...
		const num = orderedIndex || 1;
		let result = `${prefix}${num}. ${text}\n`;
		result += renderChildrenWithOrder(block, blockMap, opts, indent + 1);
		return annotateBlock(bid, result, opts);
	}
		...
		if (addOns?.record) {
			try {
				...
				if (record.data && typeof record.data === 'string') {
					// Mermaid addon: component_type_id starts with blk_631fefbb
					const isMermaid = addOns.component_type_id?.startsWith('blk_631fefbb');
					const lang = isMermaid ? 'mermaid' : 'text';
					return annotateBlock(bid, `\`\`\`${lang}\n${record.data}\n\`\`\`\n`, opts);
				}
			} catch {}
		}
		return annotateBlock(bid, `[插件](${addOns?.component_type_id || 'addon'})\n`, opts);
	}

	// --- Embedded blocks (Board, Sheet, Bitable, Mindnote) ---
	...
	if (type === BlockType.Task) {
		...
		const bullet = indent > 0 ? `${prefix}- ` : '';
		if (taskId) {
			const info = opts.entityInfo[`task:${taskId}`];
			const taskUrl = info?.href || `https://applink.feishu.cn/client/todo/detail?guid=${taskId}`;
			const label = info?.label || '任务';
			return annotateBlock(bid, `${bullet}[${label}](${taskUrl})\n`, opts);
		}
		return annotateBlock(bid, `${bullet}[任务]\n`, opts);
	}
...
/** Render children with ordered list tracking */
function renderChildrenWithOrder(
	block: FeishuBlock,
	blockMap: Map<string, FeishuBlock>,
	opts: Required<ConvertOptions>,
	indent: number,
	...
	for (const childId of children) {
		...
		const rendered = renderBlock(child, blockMap, opts, indent, orderedCounter);
