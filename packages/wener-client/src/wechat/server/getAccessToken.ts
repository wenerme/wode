...
export function getAccessToken(
	{ grant_type = 'client_credential', appid, secret }: { grant_type?: string; appid: string; secret: string },
	opts: Partial<RequestOptions> = {},
) {
	return request<AccessTokenResponse>({ ...opts, url: 'token', params: { grant_type, appid, secret } });
}
...
export function getStableAccessToken(
	...
	return request<AccessTokenResponse>({
		...opts,
		url: 'stable_token',
		body: { grant_type: 'client_credential', ...params },
	});
}

export function code2session(
	params: { grant_type?: string; appid: string; secret: string; js_code: string },
	opts: Partial<RequestOptions> = {},
) {
	return request<{ session_key: string; openid: string; unionid?: string }>({
		...opts,
