...
export class CubeClient {
	...
	async load(req: CubeLoadRequest) {
		return this.request<CubeLoadResponse>({ path: '/v1/load', body: req });
	}
	...
	async readyz() {
		return this.request<{ health: 'HEALTH' | 'DOWN' }>({ path: '/readyz' });
	}

	async livez() {
		return this.request<{ health: 'HEALTH' | 'DOWN' }>({ path: '/livez' });
	}
}
