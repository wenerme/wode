...
type PivotQuery = Query & { queryType: QueryType };
...
type UsedPreAggregation = { targetTableName: string; type: PreAggregationType };
...
export type DrillDownLocator = { xValues: string[]; yValues?: string[] };

export type Series<T> = { key: string; title: string; shortTitle: string; series: T[] };

export type Column = { key: string; title: string; series: [] };

export type SeriesNamesColumn = { key: string; title: string; shortTitle: string; yValues: string[] };

export type ChartPivotRow = { x: string; xValues: string[]; [key: string]: any };
...
export type PivotRow = { xValues: Array<string | number>; yValuesArray: Array<[string[], number]> };

export type SerializedResult<T = any> = { loadResponse: LoadResponse<T> };

export type Filter = BinaryFilter | UnaryFilter | LogicalOrFilter | LogicalAndFilter;
export type LogicalAndFilter = { and: Filter[] };

export type LogicalOrFilter = { or: Filter[] };
...
type TimeDimensionComparisonFields = { compareDateRange: DateRange[]; dateRange?: never };
export type TimeDimensionComparison = TimeDimensionBase & TimeDimensionComparisonFields;

type TimeDimensionRangedFields = { dateRange?: DateRange };
...
type DeeplyReadonly<T> = { readonly [K in keyof T]: DeeplyReadonly<T[K]> };
...
export type TCubeMeasure = BaseCubeMember & {
	...
	drillMembers: string[];
	drillMembersGrouped: { measures: string[]; dimensions: string[] };
	format?: 'currency' | 'percent';
};

export type TCubeDimension = BaseCubeMember & { primaryKey?: boolean; suggestFilterValues: boolean };
