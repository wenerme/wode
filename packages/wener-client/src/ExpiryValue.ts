...
) => Promise<{ value: T; expiresAt: number | Date }>;

export type ExpiryValue<T = string> = { expiresAt: Date; value: T };
type PartialRequired<T, K extends keyof T> = Partial<Omit<T, K>> & Required<Pick<T, K>>;

export type ExpireValueHolderInit<T = string> =
	| MaybePromise<{ value: T; expiresAt: number | Date } | undefined>
...
export function createExpireValueHolder<T = string>({
	...
	return { get: async () => val$.then(reload).then((v) => v.value), getExpiryValue: async () => val$.then(reload) };
}
...
function coerceValue<T>(o: { value: T; expiresAt: number | Date } | undefined): ExpiryValue<T> | undefined {
	...
	return { value: o.value, expiresAt: tryParseDate(o.expiresAt) };
}
