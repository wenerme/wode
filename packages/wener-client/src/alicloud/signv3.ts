...
export async function signv3(
	...
	}: { method: string; url: string; headers: Record<string, any>; body?: string | BufferSource },
	{
		...
		nonce,
		debug,
	}: { accessKeyId: string; accessKeySecret: string; date?: string; nonce?: string; debug?: boolean },
) {
	const u = new URL(url);
	headers.host ||= u.host;
	...
	return { signature, authorization, date, nonce, headers: { ...headers, authorization } };
}
