...
export interface AliCloudRequestOptions<T> {
	...
	) => MaybePromise<{ headers: Record<string, any> }>;
	onSuccess?: (ctx: {
		res: Response;
		req: RequestInit;
...
export async function request<T>(options: AliCloudRequestOptions<T>) {
	...
	const init: { method: string; headers: Record<string, any>; body?: string | BufferSource } = {
		method,
		headers,
		body: undefined,
	};
	...
	try {
		...
		if (onResponse) {
			await onResponse({ res, req: init, payload, error });
		}
	...
	data = (await onSuccess?.({ res, req: init, payload, data })) ?? data;
	return data;
}
