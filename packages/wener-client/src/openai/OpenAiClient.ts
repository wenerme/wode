...
export class OpenAiClient {
	...
	constructor({
		...
		this.options = { ...base, fetch, baseUrl, onImage, headers };
	}

	async getModels() {
		return this.request<ListObject<ModelObject>>({ url: 'models' });
	}

	async getModel(id: string) {
		return this.request<ModelObject>({ url: `models/${id}` });
	}

	async deleteModel(id: string) {
		return this.request<DeletedObject<'model'>>({ method: 'DELETE', url: `models/${id}` });
	}

	async request<T>(opts: OpenAiRequestOptions<T>) {
		const { fetch, baseUrl, headers } = this.options;
		return request({ fetch, baseUrl, ...opts, headers: { ...headers, ...opts.headers } });
	}
}
