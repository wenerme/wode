...
export class Client {
	...
	async request<T = any>(init: {
		...
		const r = { method, ...rest, headers, body: body && typeof body === 'object' ? JSON.stringify(body) : body };
		url = url.replace(/^\/+/, '');
	...
	async customerUpdate({
		...
		return this.request<string>({ url: '/manage/customer/updateCustomer', body: { ...rest } });
	}
	...
	async customerAdd({
		...
		return this.request<string>({ url: '/manage/customer/addCustomer', body: { ...rest } });
	}
	...
	async customerUpdateStatus({
		...
		return this.request<QueryCustomerItem>({ url: '/manage/customer/updateCustomerStatus', body: { ...rest } });
	}
	...
	async customerBatchAssignRoles(body: {
		...
		return this.request<QueryCustomerItem>({ url: '/manage/customer/batchAssignRoles', body: { ...body } });
	}
}
