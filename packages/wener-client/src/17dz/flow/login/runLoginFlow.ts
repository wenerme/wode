import { createChildLogger, type Logger, sleep } from '@wener/utils';
...
export async function runLoginFlow({
	...
	const { login, cookie, page: sessionPage } = await getSession({ browser, logger, page: previousPage, reload });
	if (login && !force) {
	...
	try {
		...
		return { cookie: await page.cookies(), page: close ? undefined : page };
	} catch (error) {
