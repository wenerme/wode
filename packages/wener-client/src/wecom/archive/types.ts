...
export interface ImageMessage extends SendMessage {
	msgtype: 'image';
	image: { md5sum: string; filesize: number; sdkfileid: string };
}
...
export interface WeChatChannelMessage extends SendMessage {
	msgtype: 'sphfeed';
	sphfeed: { feed_type: number; sph_name: string; feed_desc: string };
}
...
export interface VoipDocShareMessage extends SendMessage {
	...
	voip_doc_share: { filename: string; md5sum: string; filesize: number; sdkfileid: string };
}
...
export interface MettingVoiceCallMessage extends SendMessage {
	...
	meeting_voice_call: {
		...
		demofiledata: { filename: string; demooperator: string; starttime: number; endtime: number }[];
		sharescreendata: { share: string; starttime: number; endtime: number }[];
	};
}
...
export type NewsMessageItem = { title: string; description: string; url: string; picurl: string };

export interface NewsMessage extends SendMessage {
	msgtype: 'news';
	info: { item: NewsMessageItem[] };
}
...
export type ChatRecordMessageItem = { type: string; msgtime: number; content: string; from_chatroom: boolean };

export interface ChatRecordMessage extends SendMessage {
	msgtype: 'chatrecord';
	chatrecord: { title: string; item: ChatRecordMessageItem[] };
}
...
export interface LinkMessage extends SendMessage {
	msgtype: 'link';
	link: { title: string; description: string; link_url: string; image_url: string };
}

export interface FileMessage extends SendMessage {
	msgtype: 'file';
	file: { md5sum: string; filename: string; filesize: number; sdkfileid: string; fileext: string };
}
...
export interface LocationMessage extends SendMessage {
	msgtype: 'location';
	location: { longitude: number; latitude: number; address: string; title: string; zoom: number };
}

export interface CardMessage extends SendMessage {
	msgtype: 'card';
	card: { corpname: string; userid: string };
}

export interface VideoMessage extends SendMessage {
	msgtype: 'video';
	video: { md5sum: string; filesize: number; play_length: number; sdkfileid: string };
}

export interface VoiceMessage extends SendMessage {
	msgtype: 'voice';
	voice: { md5sum: string; voice_size: number; play_length: number; sdkfileid: string };
}

export interface AggreeMessage extends SendMessage {
	msgtype: 'agree';
	agree: { userid: string; agree_time: number };
}

export interface DisagreeMessage extends SendMessage {
	msgtype: 'disagree';
	disagree: { userid: string; disagree_time: number };
}
...
export type MessageTypeContent = { [K in keyof TypeContent]: { type: K; content: TypeContent[K] } }[keyof TypeContent];
...
export type ParsedChatRecordMessageItem = {
	[K in keyof ChatRecordMessageTypeContentMapping]: { type: K; content: ChatRecordMessageTypeContentMapping[K] };
}[keyof ChatRecordMessageTypeContentMapping] & { msgtime: number; from_chatroom: boolean };
