...
export type GetArchiveMessageItem = { sequence: number; corpId: string } & ArchiveMessage;
...
export class WeWorkFinanceClient {
	...
	private constructor({ sdk, ...options }: { sdk: any } & Partial<WeWorkFinanceClientOptions>) {
		this.#sdk = sdk;
		this.options = { debug: false, corpId: '', ...options };
	...
	decrypt(opts: { decryptKey?: string | Buffer; randomKey?: string; privateKey?: string; encrypted: string }): string {
		return decrypt({ privateKey: this.options.privateKey, ...opts });
	}
	...
	getMessage(req: GetChatDataRequest): GetArchiveMessageItem[] {
		...
		for (let ele of res.chatdata) {
			let dec: string;
			try {
				dec = this.decrypt({ randomKey: ele.encrypt_random_key, encrypted: ele.encrypt_chat_msg });
			} catch (e) {
				this.log.error(
					[
				...
				this.debug(() => {
					return [
						'payload',
						JSON.stringify({ randomKey: ele.encrypt_random_key, encrypted: ele.encrypt_chat_msg }),
					].join();
				});
				throw e;
			}
...
export function decrypt({
	...
	if (!decryptKey && randomKey && privateKey) {
		decryptKey = decryptRandomKey({ randomKey, privateKey, native: true });
	}
	if (!decryptKey) {
