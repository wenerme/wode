...
export type ClientOptionsInit = Partial<ClientOptions> & { debug?: boolean };
...
export class Client {
	...
	constructor({ debug, ...options }: ClientOptionsInit = {}) {
		this.options = { ...options, fetch: debug ? createFetchWithLogging({ fetch: options.fetch }) : options.fetch };
	}

	async request<T>(o: Omit<RequestOptions, 'token'> & { token?: TokenProvider | boolean }) {
	...
	async getReceiptImages(params: {
		...
		return this.request<GetReceiptImagesResponseItem[]>({
			...
			method: 'GET',
			params: { statusCode: '2', hzrwType: 1, ...params },
		});
	}
