...
export interface SequelizeWhereOptions {
	...
	functions?: Record<string, { arity?: number }>;
}
...
function resolve(parts: string[], c: WhereContext) {
	...
	while (rest.length) {
		...
		if (assoc) {
			...
			if (!include) {
				include = { association: assoc, required: true };
				c.include.push(include);
			} else {
				// nested
				include.include = [{ association: assoc, required: true }];
			}
	...
	const setOp = (left: MiniQueryASTNode, op: any, val: any) => {
		...
		switch (left.type) {
			...
			case 'call': {
				...
				current = where(fn(name, col(cn)), { [op]: val });
				return;
			}
			case 'ref':
				// 依然无法 escape `.`
