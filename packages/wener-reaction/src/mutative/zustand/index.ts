...
type StoreMutative<S> = S extends { getState: () => infer T; setState: infer SetState }
...
type MutativeImpl = <T>(storeInitializer: StateCreator<T, [], []>) => StateCreator<T, [], []>;

const mutativeImpl: MutativeImpl = (initializer) => (set, get, store) => {
	type T = ReturnType<typeof initializer>;

	store.setState = (updater, replace, ...a) => {
		const nextState = (typeof updater === 'function' ? create(updater as any) : updater) as
			| ((s: T) => T)
			| T
			| Partial<T>;
