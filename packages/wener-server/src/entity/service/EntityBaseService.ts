...
export class EntityBaseService<E extends StandardBaseEntity> extends BaseEntityService<E> implements EntityService<E> {
	...
	async getEntity(
		...
		{ build, lockMode }: { build?: (builder: QueryBuilder<E>) => void; lockMode?: LockMode } = {},
	) {
		const { id } = req;
		let { builder } = await this.createQueryBuilder();
	...
	async patch(req: PatchEntityRequest) {
		return this.em.transactional(async (em) => {
			const { data } = req;
			const entity = await this.getEntity(req, { lockMode: LockMode.PESSIMISTIC_WRITE });
			const before = entity.toPOJO();
			{
	...
	async delete(r: DeleteEntityRequest): Promise<GeneralResponse<E>> {
		...
		if (!entity) {
			...
			return { status: 200, message: '资源不存在' };
		}
		...
		writeEntityAuditLog({ entity, action: EntityAuditAction.Delete, em });
		await em.flush();
		return { status: 200, message: '已删除', data: entity };
	}
