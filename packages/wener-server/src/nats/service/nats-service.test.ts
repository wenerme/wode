...
const triggers = { server: 0, client: 0 };
describe('nats service module', async () => {
	...
	beforeAll(async () => {
		...
		{
			const moduleRef = await Test.createTestingModule({ imports: [ServerModule] }).compile();
		...
		{
			const moduleRef = await Test.createTestingModule({ imports: [ClientModule] }).compile();
...
@Service({ name: 'wener.test.TestService' })
...
@ExposeService({ as: AbstractTestService })
