...
export class ServiceRegistry {
	...
	addService({ service, target }: RegisterServiceOptions) {
		...
		this.services[name] = { name, target, metadata };
	}

	private readonly _handle = async (req: ServerRequest): Promise<ServerResponse> => {
		const svc = this.services[req.service];
		const ctx: ServerRequestOptions = { id: req.id, metadata: req.metadata, headers: req.headers, options: {} };
		if (!svc) {
		...
		return createResponseFromRequest(req, { body: output });
	};
	...
	async handle(req: ServerRequest): Promise<ServerResponse> {
		...
		try {
			...
			return createResponseFromRequest(req, { error, status: 500, description: String(error) });
		}
	}
}
