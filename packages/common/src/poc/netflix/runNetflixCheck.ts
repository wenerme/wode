import { verifyNetflixProxy, type VerifyResult } from '@/poc/netflix/verifyNetflixProxy';
...
export async function runNetflixCheck({ proxy, username, password, checkAll }: RunNetflixCheckOptions) {
	...
	let out: { id: string; proxy: string; result: VerifyResult | undefined } | undefined;
	for (let proxy of all) {
		...
		const result = await verifyNetflixProxy({ proxy });
