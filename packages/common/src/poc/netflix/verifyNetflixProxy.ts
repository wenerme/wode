...
export async function verifyNetflixProxy({ proxy, ...options }: VerifyOptions): Promise<VerifyResult> {
	...
	const result: VerifyCheckResult = { Available: true, SelfMade: true, NonSelfMade: true };
	const out: VerifyResult = { country: '', check: result };
	const init: Record<string, any> = { redirect: 'manual' };
