 ...
 */
class ReadableWebToNodeStream extends Readable {
...
export function toNodeReadableStream(rs: ReadableStream): MaybePromise<Readable> {
  // https://github.com/Borewit/readable-web-to-node-stream/blob/master/lib/index.ts
  if (typeof process !== 'undefined' && process.release?.name === 'node') {
    return import('node:stream')
      .then((s) => s.Readable.fromWeb(rs as any) as Readable)
      .catch(() => {
        return new ReadableWebToNodeStream(rs);
      });
  }
  return new ReadableWebToNodeStream(rs);
}
