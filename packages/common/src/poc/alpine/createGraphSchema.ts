...
export function createGraphSchema() {
	...
	builder.queryField('hello', (t) => {
		return t.string({ args: { name: t.arg.string() }, resolve: (parent, { name }) => `Hello, ${name || 'World'}` });
	});
...
function createApkIndexSchema({ builder }: { builder: SchemaBuilderType }) {
	...
	builder.queryFields((t) => {
		return {
           ...
           */
					const out = await svc.list({ limit: first || 30 });
					return {
						pageInfo: { hasNextPage: false, hasPreviousPage: false, startCursor: 'abc', endCursor: 'def' },
						edges: out.data.map((v) => {
							return { node: v, cursor: `A${v.id}` };
						}),
					};
				},
			}),
			...
			fetchApkIndex: t.field({
				...
				resolve: async (root, { branch, arch, repo }) => {
					const { found, changed } = await fetchApkIndex({
						em: getEntityManager(),
						variants: [{ branches: [branch], architectures: [arch], repos: [repo] }],
					});
