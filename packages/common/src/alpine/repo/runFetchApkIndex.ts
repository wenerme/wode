...
export interface FetchApkIndexOptions {
	force?: boolean;
	variants: { branches: string[]; architectures: string[]; repos: string[] }[];
	em: EntityManager;
}

export async function runFetchApkIndex({ variants, em, force }: FetchApkIndexOptions) {
	// get path list
	...
	return { stats, changed: changedEntities, found: foundAll };
}
