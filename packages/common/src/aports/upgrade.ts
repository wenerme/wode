...
import { argv, chalk, type ProcessOutput } from 'zx';
import 'zx/globals';
...
if (ids.length === 1) {
	...
	for (const id of ids) {
		await upgrade({ pkg: id, dry: parseBoolean(argv['dry-run']) });
	}
}
...
async function upgrade({ pkg, dry = false, next }: { pkg: ParsedPackageId; dry?: boolean; next?: string }) {
	...
	try {
		await fs.writeFile('APKBUILD.next', await writeApkBuild({ ver: next, rel: 0, write: false }));
