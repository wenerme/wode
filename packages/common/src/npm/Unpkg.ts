...
export class Unpkg {
		...
		if (!(await storage.hasPackageFile(pkgId))) {
			...
			readable
				...
				.on('entry', async (entry: ReadEntry) => {
					...
					await storage.savePackageFile({ package: pkgId, size: entry.size ?? 0, path: p, data: buf });
				});
			await read;
		}
	...
	async getPackageInfo(packageName: string): Promise<RegistryPackage> {
		...
		try {
			...
			if (mtime) {
				if (Date.now() - mtime.getTime() < 1000 * 60 * 60 * 15) {
					...
					out = JSON.parse(text);
					lru.set(name, out, { size: text.length, ttl: this.options.packageIndexTtlMs });
					return out;
				} else {
		...
		lru.set(name, out, { size, ttl: this.options.packageIndexTtlMs });
	...
	async getPackageVersionInfo(packageName: string): Promise<RegistryPackageJson> {
		...
		if (range !== 'latest') {
			...
			if (meta) {
				out = JSON.parse(meta);
				lru.set(id, out, { size: meta.length, ttl: this.options.packageVersionTtlMs });
				return out;
			}
		}
		...
		await storage.savePackage({ name, version, meta: JSON.stringify(out) });
		{
