...
const meta = {
	id: 'utilities-formats',
	title: 'Core/Components/Formats',
	parameters: {
		layout: 'fullscreen',
	},
} satisfies Meta;
						...
						<tbody>
							{rows.map((row) => (
								<tr key={row.label} className='border-base-300 border-t'>
									<th className='px-3 py-3 font-medium break-words'>{row.label}</th>
									<td className='min-w-0 px-3 py-3 break-words'>{row.value}</td>
