...
import { FileManager, renderFileManagerWindow, showFileManager } from '@/file/file-manager';
...
export function ConsoleFilesContent({ fileSystem }: { fileSystem?: IFileSystem }) {
	return fileSystem ? (
		<FileManager showHeader={false} className='h-full min-h-0 border-0' fileSystem={fileSystem} title='工作区文件' />
	) : (
