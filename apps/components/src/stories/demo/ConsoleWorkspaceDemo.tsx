...
} from '@/console/console-preferences';
import {
	ConsoleActivityItem,
	ConsoleActivityList,
	ConsoleContent,
...
export type ConsoleWorkspaceDemoProps = {
	...
	resourceCounts?: Partial<Record<DataListPage, number>>;
...
export function ConsoleWorkspaceDemo({
	...
	const dataListPage = enableWorkspace && isDataListPage(page);
	const mobileNavigation = enableWorkspace ? (
		<nav aria-label='当前模块导航' className='flex w-full gap-1 overflow-x-auto md:hidden'>
			{navigation.map((item) => {
				const nextPage = item.key as DemoPage;
				const liveCount = isDataListPage(nextPage) ? resourceCounts?.[nextPage] : undefined;
				const badge = liveCount === undefined ? ('badge' in item ? item.badge : undefined) : String(liveCount);
				return (
					<a
						key={item.key}
					...
					>
						...
						{item.label}
						{badge ? <span className='text-[10px] tabular-nums opacity-70'>{badge}</span> : null}
					</a>
				);
			})}
		</nav>
	...
	return (
		<>
			...
			<ConsoleShell
							...
							{navigation.map((item) => {
								...
								return (
									<ConsoleNavLink
										...
										badge={liveCount === undefined ? ('badge' in item ? item.badge : undefined) : String(liveCount)}
										collapsed={collapsed}
				...
				header={
					(enableWorkspace && page === 'files') || page === 'resources' ? null : dataListPage ? (
						<ConsoleHeader className='min-h-0 px-2 py-1 md:hidden'>{mobileNavigation}</ConsoleHeader>
					) : (
						<ConsoleHeader
							breadcrumbs={
								enableWorkspace
			...
			>
				<ConsoleContent
					className={
						page === 'workspace' || page === 'files' || page === 'resources' || dataListPage
							? 'relative overflow-hidden'
							: undefined
					}
				>
					{page === 'home' && enableWorkspace ? (
						<ConsoleRoutedHomeContent counts={resourceCounts} hrefForPage={hrefForPage} onNavigate={changePage} />
					) : null}
					{page === 'home' && !enableWorkspace ? (
					...
					{enableWorkspace && routedModulePage && page !== 'account' && page !== 'contact' ? (
						<ConsoleRoutedModuleContent key={routedModulePage} page={routedModulePage} />
					) : null}
...
function ResourcesContent() {
	return <DataViewResourceWorkspaceDemo />;
}
