...
import { createWindowManagerStore, WindowManagerHost, WindowManagerProvider } from '@/window/window-manager';
...
export function ConsoleDemo({
	...
	const [automationDatabaseName] = useState(() =>
		dataPartition === 'automation' ? createConsoleDemoAutomationDatabaseName() : undefined,
	);
	...
	if (screen === 'login') {
		return (
			<LoginPage.Composite
				...
				formMode='email'
				defaultValues={{ email: account.email, password: account.password }}
	...
	return (
		<ConsoleDemoWorkspace
			databaseName={databaseName ?? automationDatabaseName}
...
function RoutedConsoleRuntime({
	...
	const {
		...
		retry: retryResourceCounts,
	} = useConsoleDemoResourceCounts(runtime);
	return (
				...
				background={
					<div className='absolute inset-y-0 right-[58px] left-0'>
						{resourceCountError ? (
							<div
								role='alert'
								className='border-error/40 bg-base-100 absolute top-3 left-1/2 z-50 flex max-w-[calc(100vw-2rem)] -translate-x-1/2 items-center gap-3 border px-3 py-2 shadow-lg'
							>
								<span className='text-sm'>{resourceCountError}</span>
								<button type='button' className='btn btn-error btn-xs' onClick={retryResourceCounts}>
									重试
								</button>
							</div>
						) : null}
...
function useConsoleDemoResourceCounts(runtime: ConsoleDemoDataRuntime) {
	const [counts, setCounts] = useState<ConsoleDemoResourceCounts>({
		account: CONSOLE_DEMO_COUNTS.customers,
		contact: CONSOLE_DEMO_COUNTS.contacts,
	});
	const [error, setError] = useState<string>();
	const [revision, setRevision] = useState(0);
	useEffect(() => {
		return subscribeConsoleDemoResourceCounts({
			read: async () => ({
				account: await runtime.database.customers.where('partition').equals(runtime.partition).count(),
				contact: await runtime.database.contacts.where('partition').equals(runtime.partition).count(),
			}),
			onCounts: (next) => {
				setCounts(next);
				setError(undefined);
			},
			onError: () => setError('资源数量同步失败'),
		});
	}, [revision, runtime]);
	return { counts, error, retry: () => setRevision((value) => value + 1) };
}
