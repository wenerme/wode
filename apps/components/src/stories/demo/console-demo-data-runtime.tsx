...
import { CONSOLE_DEMO_DATABASE_NAME, type ConsoleDemoPartition } from './console-demo-database';
...
export function ConsoleDemoDataProvider({
	children,
	databaseName,
	partition,
	resetOnMount = false,
}: ConsoleDemoDataProviderProps) {
	...
	useEffect(() => {
		...
		return () => {
			...
			if (runtime) scheduleConsoleDemoRuntimeDispose(runtime);
		};
	}, [databaseName, partition, resetOnMount, retry]);
...
export function scheduleConsoleDemoRuntimeDispose(
	runtime: { dispose: () => Promise<void> },
	schedule: (callback: () => void) => void = queueMicrotask,
) {
	schedule(() => void runtime.dispose());
}

export function createConsoleDemoAutomationDatabaseName(createId: () => string = () => crypto.randomUUID()) {
	return `${CONSOLE_DEMO_DATABASE_NAME}-automation-${createId()}`;
}
