...
export function ConsoleRailLink({ href, active, icon, label, className, ...props }: ConsoleRailLinkProps) {
	return (
		<a
			...
			data-tip={label}
			className={cn(
				'tooltip tooltip-bottom text-base-content/65 md:tooltip-right grid size-10 shrink-0 place-items-center rounded-md transition-colors outline-none',
...
export function ConsoleSidebarHeader({ className, ...props }: ConsoleSidebarHeaderProps) {
	return (
		<div
			className={cn('border-base-300 flex h-[57px] shrink-0 items-center gap-3 border-b px-3', className)}
			{...props}
		/>
	);
}
