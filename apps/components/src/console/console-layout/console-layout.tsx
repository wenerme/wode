...
export namespace ConsoleLayout {
	...
	export type CompositeProps = Omit<ComponentPropsWithRef<'div'>, 'children'> & {
		menu?: ReactNode;
		dock?: ReactNode;
		children?: ReactNode;
	};
	...
	export function Root({ children, className, ...props }: RootProps) {
		return (
			<div className={cn('flex h-dvh w-full overflow-hidden', 'flex-col md:flex-row', className)} {...props}>
				{children}
			</div>
		);
	}
	...
	export function Menu({ children, className, ...props }: MenuProps) {
		return (
			<aside
				className={cn(
					'border-base-300 bg-base-100 flex items-center',
	...
	export function Main({ children, className, ...props }: MainProps) {
		return (
			<main className={cn('relative order-2 min-h-0 min-w-0 flex-1 overflow-hidden', className)} {...props}>
				<div className='absolute inset-0 isolate scrollbar-thin overflow-auto'>{children}</div>
			</main>
		);
	}
