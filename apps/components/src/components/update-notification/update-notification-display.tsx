		...
		<>
			...
			{notification.open && (
				<section
					{...props}
					aria-labelledby={titleId}
					className={cn('border-base-300 bg-base-100 text-base-content border-y px-4 py-3', className)}
				>
...
function VersionTransition({ notification, labels, className }: VersionTransitionProps) {
	...
	return (
		<div className={cn('flex min-w-0 flex-wrap items-center gap-2 text-xs tabular-nums', className)}>
			<span className='flex min-w-0 items-center gap-2'>
				<span className='min-w-0'>
					<span className='text-base-content/70'>{labels.currentVersion}</span>{' '}
					<code className='font-mono break-all'>{notification.currentVersion}</code>
				</span>
			...
			<span className='min-w-0'>
				...
				<code className='text-base-content font-mono font-semibold break-all'>{notification.latestVersion}</code>
			</span>
		</div>
	);
}
...
function DismissButton({ notification, label, className }: NotificationActionProps) {
	return (
		<button
			type='button'
			className={cn('btn btn-ghost btn-sm btn-square shrink-0', className)}
