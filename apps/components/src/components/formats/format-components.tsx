...
export function EmptyPlaceholder({ as: Component = 'span', children, className, ...props }: EmptyPlaceholderProps) {
	...
	return (
		<Component className={cn('text-base-content/60', className)} {...props}>
...
export function TruncateFormat({
	...
	return (
		<Component className={cn('inline-block max-w-full truncate align-bottom', className)} title={textTitle} {...props}>
			{content}
		</Component>
	);
}
...
export function PhoneNumberFormat({
	...
	if (isFallbackText(text, fallback)) return <>{resolvePlaceholder(placeholder)}</>;
	return (
		<span {...props} className={cn('font-mono tabular-nums', className)}>
			{text}
		</span>
	);
}
