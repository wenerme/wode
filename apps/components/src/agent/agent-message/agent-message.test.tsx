...
describe('AgentMessage', () => {
	it('renders streaming content and structured parts without default role or numbered step chrome', () => {
		...
		expect(markup).not.toContain('data-slot="message-header"');
	...
	it('keeps role headers and numbered steps as explicit opt-ins', () => {
		const markup = renderToStaticMarkup(
			<AgentMessage
				message={message('assistant-step', 'assistant', [{ type: 'step-start' }, { type: 'text', text: '结果' }])}
				showHeader
				showSteps
			/>,
		);
		expect(markup).toContain('data-slot="message-header"');
		expect(markup).toContain('助手');
	...
	it('uses plain role-free defaults while retaining screen-reader cues and injectable renderers', () => {
		...
		expect(user).not.toContain('data-slot="message-header"');
		expect(user).toContain('class="sr-only">你</span>');
		expect(user).toContain('bg-muted');
		expect(user).not.toContain('bg-primary');
		expect(system).not.toContain('data-slot="message-header"');
		expect(system).toContain('class="sr-only">系统</span>');
