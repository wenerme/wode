...
import { createOpenAICompatibleDirectTransport, type OpenAICompatibleReasoningEffort } from './direct-transport';
...
function AiSdkAgentChatSession({
	...
	const transport = useMemo(
		...
		[connection, fetch, instructions, maxSteps, reasoning, tools, transportLimits],
	);
	...
	const [optimisticSubmitting, setOptimisticSubmitting] = useState(false);
	const mounted = useRef(false);
	const fileController = useRef<AbortController | undefined>(undefined);
	const submissionSequence = useRef(0);
	const optimisticSubmission = useRef<number | undefined>(undefined);
	...
	useEffect(() => {
		if (chatMessages !== appliedExternalMessages.current) {
			...
			optimisticSubmission.current = undefined;
	...
	async function send(input: { files: File[]; text: string }) {
		...
		if (optimisticSubmission.current !== undefined) return;
		const submission = ++submissionSequence.current;
		optimisticSubmission.current = submission;
		...
		try {
			...
			setFiles([]);
			void request.then(
				() => finishOptimisticSubmission(submission),
				() => finishOptimisticSubmission(submission),
			);
		} catch (value) {
			...
			if (!requestStarted) finishOptimisticSubmission(submission);
			if (fileController.current === controller) fileController.current = undefined;
		}
	}

	function finishOptimisticSubmission(submission: number) {
		if (optimisticSubmission.current !== submission) return;
		optimisticSubmission.current = undefined;
		if (mounted.current) setOptimisticSubmitting(false);
	}
	...
	return (
		<AgentChat
			...
			status={optimisticSubmitting && status === 'ready' ? 'submitted' : status}
			text={text}
			value={messages}
			onFilesChange={(next) => {
				setLocalError(undefined);
