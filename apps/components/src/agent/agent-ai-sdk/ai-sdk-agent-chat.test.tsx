...
describe('AiSdkAgentChat connection lifecycle', () => {
	...
	it('restores the mounted guard under StrictMode and sends after asynchronous file conversion', async () => {
		...
		await waitForAct(() => expect(fetch).toHaveBeenCalledOnce());
		await waitForAct(() => expect(container.textContent).toContain('严格模式回复'));
	});

	it('shows pending feedback immediately while the provider response is unresolved', async () => {
		...
		await typeIntoTextarea('等待中的请求');
		await act(async () => {
			container.querySelector<HTMLButtonElement>('button[aria-label="发送"]')?.click();
			await Promise.resolve();
		});
		expect(container.querySelector('[data-slot="agent-chat-pending"]')).not.toBeNull();
		expect(container.querySelector('[role="status"]')?.getAttribute('aria-label')).toBe('模型响应处理中');
		expect(container.querySelector('[role="status"]')?.textContent).toMatch(/^\d+\.\d+s$/u);
		expect(container.querySelector<HTMLButtonElement>('button[aria-label="停止生成"]')).not.toBeNull();
		expect(fetch).toHaveBeenCalledOnce();

		await act(async () => {
			resolveFetch?.(streamResponse('延迟回复'));
			await new Promise((resolve) => setTimeout(resolve, 30));
		});
		expect(container.textContent).toContain('延迟回复');
		expect(container.querySelector('[data-slot="agent-chat-pending"]')).toBeNull();
	});

	it('clears pending feedback when the user stops an unresolved request', async () => {
		const signals: AbortSignal[] = [];
		const fetch = vi.fn((_input: RequestInfo | URL, init?: RequestInit) => {
			const signal = init?.signal;
			if (signal) signals.push(signal);
		...
		await act(async () => {
			root.render(
				<AiSdkAgentChat
					...
					fetch={fetch}
					transportLimits={{ maxRetries: 0 }}
				/>,
			);
		});
		await typeIntoTextarea('停止这个请求');
		await act(async () => {
			container.querySelector<HTMLButtonElement>('button[aria-label="发送"]')?.click();
			await Promise.resolve();
		});
		...
		expect(signals[0]?.aborted).toBe(true);
		expect(container.querySelector('[data-slot="agent-chat-pending"]')).toBeNull();
		expect(container.querySelector<HTMLButtonElement>('button[aria-label="发送"]')).not.toBeNull();
	});

	it('clears pending feedback when the provider fails', async () => {
...
async function waitForAct(assertion: () => void, timeout = 2_000) {
	...
	throw failure ?? new Error('condition was not met');
}
