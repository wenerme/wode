...
describe('file system file viewer boundaries', () => {
	...
	it('reads legal filesystem names containing URL delimiter characters', async () => {
		...
		expect(loaded.fileType?.id).toBe('text');
	...
	it('reads bounded bytes for a scoped custom viewer without exposing filesystem access to the renderer', async () => {
		const manager = createFileManager();
		manager.fileTypes.register({
			id: 'custom-binary',
			label: '自定义二进制',
			match: { extensions: ['custom'] },
			priority: 1000,
			viewer: { render: ({ bytes }) => <span>{bytes instanceof Uint8Array ? bytes.byteLength : 0}</span> },
		});
		const readFile = vi.fn(async () => new Uint8Array([1, 2, 3]));
		const loaded = await loadFileSystemFileViewer({
			fileSystem: { readFile, stat: vi.fn(async () => ({ kind: 'file' as const, size: 3 })) },
			manager,
			maxBytes: 3,
			path: '/workspace/value.custom',
		});
		expect(loaded.kind).toBe('unsupported');
		expect(loaded.bytes).toEqual(new Uint8Array([1, 2, 3]));
		expect(readFile).toHaveBeenCalledWith('/workspace/value.custom', {
			encoding: 'binary',
			maxBytes: 4,
			signal: undefined,
		});
