...
export function FileSystemFileViewer({
	...
	const source = { fileSystem, manager, maxBytes, path };
	...
	const stateIsCurrent =
		...
		Object.is(state.maxBytes, maxBytes) &&
		state.path === path;
	...
	useEffect(() => {
		...
		setState({ fileSystem, manager, maxBytes, path, status: 'loading' });
		void loadFileSystemFileViewer({ fileSystem, manager, maxBytes, path, signal: controller.signal }).then(
			(loaded) => {
				if (!isFileViewerRequestCurrent(currentRequestId, requestId.current, controller.signal)) return;
				setState({ fileSystem, loaded, manager, maxBytes, path, status: 'ready' });
				callbacks.current.onLoad?.(loaded);
			},
			(error) => {
				if (!isFileViewerRequestCurrent(currentRequestId, requestId.current, controller.signal)) return;
				setState({ error, fileSystem, manager, maxBytes, path, status: 'error' });
	...
	}, [fileSystem, manager, maxBytes, path]);
	...
	return (
		<AddressableFrame className={cn('min-h-72', className)} {...props}>
			...
			<AddressableFrameContent className='bg-base-100'>
				{viewState.status === 'loading' ? (
					...
					<FileViewer
						...
						fileType={viewState.loaded.fileType}
						manager={manager}
						messages={messages}
			...
			{viewState.status === 'ready' ? (
				<AddressableFrameFooter>
					<span>
						{viewState.loaded.file.mimeType ||
							getFileSystemViewerMimeType(viewState.loaded.file, viewState.loaded.kind, manager)}
					</span>
...
export async function loadFileSystemFileViewer({
	...
	const fileType = resolveFileSystemViewerType(file, manager);
	const kind = toFileViewerKind(fileType);
	if (!fileType?.viewer && !fileType?.editor) return { file, fileType, kind };
	...
	return kind === 'text'
		? { file: actualFile, fileType, kind, text: new TextDecoder().decode(bytes) }
		: { bytes, file: actualFile, fileType, kind };
}
