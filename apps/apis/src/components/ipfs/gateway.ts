...
export function getIpfsPublicGateways() {
...
export function useIpfsGatewayState(): IpfsGatewayState {
  ...
  return ((globalThis as any).IpfsGatewayState ||= (() => {
    let _initialState: IpfsGatewayState = {};
    try {
      _initialState = JSON.parse(globalThis?.localStorage.getItem('IpfsGatewayState') || '{}');
    } catch (e) {}
    const state = proxyWithCompare({
      ..._initialState,
      prefer: 'https://ipfs.io/ipfs/:hash',
    });
