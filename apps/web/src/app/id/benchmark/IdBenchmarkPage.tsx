...
export const IdBenchmarkPage = () => {
	...
	const doBenchmark = async (n: number) => {
		const measures = idBenchmarkPage
			.filter((v) => v.next)
			.map((v) => ({ run: v.next!, name: v.name, time: 0, rate: 0 }));
		const loop = n / 10;
