...
export const PasswordHashContent = () => {
	const [state, update] = useMutative(() => {
		...
		return { hash: hash } as { hash: string; parsed?: DetectResult; verify?: boolean };
	});
	return (
...
function renderResult(result: DetectResult) {
	...
	function renderPhc() {
		...
		const cols = [
			...
			{ title: 'Salt', value: salt && renderBinary(salt) },
			{ title: 'Hash', value: hash && renderBinary(hash) },
		];

		Object.entries(params || {}).forEach(([k, v]) => {
			cols.push({ title: String(k), value: String(v) });
		});
...
type DetectResult = { format?: 'phc' | 'plain'; algorithm: string; hash: string; parsed?: Password.ParsedPassword };
...
async function parsePassword(hash: string): Promise<DetectResult | undefined> {
	if (isMd5(hash)) {
		return { hash, format: 'plain', algorithm: 'md5' };
	}
	if (isSha256(hash)) {
		return { hash, format: 'plain', algorithm: 'sha256' };
	}

	if (hash.startsWith('$')) {
		try {
			const out = await Password.parse(hash);
			return { hash, format: 'phc', algorithm: PasswordIds[out.id] || out.id, parsed: out };
		} catch (e) {
