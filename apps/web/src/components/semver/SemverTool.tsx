...
export const SemverTool: FC<{ value?: string; onValueChange?: (v: string) => void }> = ({ value, onValueChange }) => {
	const version = value || '';
...
const VersionInfo: React.FC<{ version: SemVer; onChange?: (v: string) => void }> = ({ version }) => {
	...
	const nextItems = [
		{ label: 'Major', value: new SemVer(raw).inc('major').format() },
		{ label: 'Pre-Major', value: new SemVer(raw).inc('premajor').format() },
		{ label: 'Minor', value: new SemVer(raw).inc('minor').format() },
		{ label: 'Pre-Minor', value: new SemVer(raw).inc('preminor').format() },
		{ label: 'Patch', value: new SemVer(raw).inc('patch').format() },
		{ label: 'Pre-Patch', value: new SemVer(raw).inc('prepatch').format() },
		{ label: 'Pre-Release', value: new SemVer(raw).inc('prerelease').format() },
	];
	return (
