			...
			<div className={'container mx-auto py-8'}>
				<header className={'pb-4'}>
					...
					<div className={'flex gap-2'}>
						<Link href={{ query: { tags: null } }} className={cn('badge', !tags.length && 'badge-primary')}>
						...
						{getEntryTags().map(({ name, title, icon }) => {
							...
							return (
								<Link
									key={name}
									href={{ query: { tags: next?.length ? next : null } }}
									className={cn('badge', active && 'badge-primary')}
								>
									{flexRender(icon, { className: cn('size-4') }, true)} {title}
								</Link>
							);
						})}
					</div>
