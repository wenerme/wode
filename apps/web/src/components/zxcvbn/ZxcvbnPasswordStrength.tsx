...
export const ZxcvbnPasswordStrength: React.FC<{ password?: string; data?: ZXCVBNResult }> = ({ password, data }) => {
	return (
		<div className={'p-2'}>
...
const ZxcvbnDescription: React.FC<{ data: ZXCVBNResult; password: string }> = ({ data, password }) => {
	...
	const crackTimes = [
		{ label: '100 个每小时', speedTip: '被限流的在线破解速度', value: times.online_throttling_100_per_hour },
		{ label: '10 个每秒', speedTip: '未被限流的在线破解速度', value: times.online_no_throttling_10_per_second },
		{
			label: '10,000 个每秒',
