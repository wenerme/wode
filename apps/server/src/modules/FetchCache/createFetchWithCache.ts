...
export function createFetchWithCache<T extends BaseHttpRequestLogEntity>({
	...
	return async (url, { $cache, ...init }: RequestInit & { $cache?: FetchCacheOptions } = {}) => {
		...
		try {
			...
			if (error instanceof Error) {
				e.properties.error = { message: error.message, stack: error.stack };
			} else {
				e.properties.error = { message: String(error) };
			}
