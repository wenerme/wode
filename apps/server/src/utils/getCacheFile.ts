...
export async function getCacheFile<M extends {} = Record<string, any>>({
	...
	getContent: () => MaybePromise<{ content?: Buffer | Stream | ReadableStream<Uint8Array>; metadata?: M } | void>;
}): Promise<{ file: string; metadata: M; readStream(): ReadableStream; read(): Promise<Buffer> }> {
