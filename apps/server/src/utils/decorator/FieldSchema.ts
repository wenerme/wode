...
export function getEntitySchema<T = unknown>(type: Constructor<T> | AbstractConstructor<T>): EntitySchema {
	return computeIfAbsent(_cache, type, () => {
		...
		const sc: EntitySchema = { className: meta.className, tableName: meta.tableName, fields: [] };
		protos.reverse();
		for (const proto of protos) {
			...
			for (let [k, p] of Object.entries((meta.properties ?? {}) as Record<string, EntityProperty>)) {
				...
				let fs: EntityFieldSchema = { name: k, type: '', nullable: p.nullable ?? true };
...
export type EntitySchema = { className: string; tableName: string; fields: EntityFieldSchema[] };

type EntityFieldSchema = { name: string; type: string; format?: string; nullable: boolean } & FieldSchemaOptions;
