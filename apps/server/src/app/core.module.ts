...
export class CoreModule {
	...
	static forRoot(opts: CoreModuleOptions): DynamicModule {
		...
		const mod: Required<DynamicModule> = {
			...
			module: CoreModule,
			providers: [{ provide: dayjs, useValue: getDayjs() }] as Provider[],
			exports: [dayjs],
		};
		...
		if (opts.db !== false) {
			imports.push(
				MikroOrmModule.forRootAsync({
					useFactory: (cs: ConfigService, hm: HookManager) => {
						...
						let config = createMikroOrmConfig({ clientUrl: dsn, debug, entities: [], ...db }) as MikroOrmConfig;
			...
			// postgres
			providers.push(
				{ provide: EntityManager, useExisting: CoreEntityManager },
				{
					provide: knex,
			...
			providers.push({ provide: MikroORM, useExisting: CoreMikroORM });
