...
export async function serveExternalProxy({
	...
	const {
		...
		getContent: async () => {
			...
			return { content: res.body!, metadata: { headers: Object.fromEntries(res.headers.entries()) } };
		},
	});

	return new Response(readStream(), { status: 200, headers: dropProxyHeaders(new Headers(headers)) });
}
