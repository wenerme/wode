...
export function createBaseRoute() {
	...
	app.openapi(
		createRoute({
			...
			responses: {
				200: {
					content: { 'application/json': { schema: z.object({ success: z.boolean(), now: z.string() }) } },
					description: 'Ping',
				},
			},
		}),
		(c) => {
			return c.json({ success: true, now: new Date() });
		},
	);
	...
	app.openapi(
		createRoute({
			...
			method: 'get',
			path: '/ip',
			responses: { 200: { content: { 'text/plain': { schema: z.string() } }, description: 'Get IP' } },
		}),
		(c) => {
