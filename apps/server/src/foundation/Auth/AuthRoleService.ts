...
import { getEntityDef, type StandardBaseEntity } from '@wener/server/entity';
...
export class AuthRoleService extends CustomBaseEntityService<AuthRoleEntity> {
	...
	async revoke({ entityId, roleId }: { entityId: string; roleId: string }) {
		...
		const rel = await this.em
			.getRepository(AuthEntityRoleEntity)
			.findOne({ related: role, entityId: entity.id, entityType: def.typeName });
	...
	async grant({ entityId, roleId }: { entityId: string; roleId: string }) {
		...
		await this.em
			.getRepository(AuthEntityRoleEntity)
			.upsert({ tid: role.tid, related: role, entityId: entity.id, entityType: def.typeName });
