...
export interface CreateTscPluginOptions {
 ...
 */
export function createTscPlugin({
  tsconfigPath = path.join(process.cwd(), './tsconfig.json'),
  force: forceTsc = false,
  tsx = true,
}: CreateTscPluginOptions = {}): Plugin {
  return {
    name: 'tsc',
    setup(build) {
      ...
      build.onLoad({ filter: tsx ? /\.tsx?$/ : /\.ts$/ }, async (args) => {
        ...
        if (
          !forceTsc &&
          (!parsedTsConfig || !parsedTsConfig.options || !parsedTsConfig.options.emitDecoratorMetadata)
        ) {
          return;
        }
