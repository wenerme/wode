...
declare module '@tiptap/core' {
	...
	interface NodeConfig<Options = any, Storage = any> {
		renderMarkdown?: (
			this: { name: string; options: Options; storage: Storage },
	...
	interface Commands<ReturnType> {
		markdown: { setMarkdownContent: (content: string) => ReturnType };
	}
}
...
export const MarkdownExtension = Extension.create<MarkdownOptions>({
	name: 'markdown',
	addOptions() {
		return { serializers: { nodes: {}, marks: {} } };
	},
...
const extractRenderMarkdown = (nodesOrMarks: Record<string, MarkType | NodeType>) => {
	return Object.entries(nodesOrMarks)
		...
		.reduce((items, [name, renderMarkdown]) => ({ ...items, [name]: renderMarkdown }), {});
};
