...
export type SubTextStylesOptions = { types: string[]; defaultUnit: string };
...
export const LineHeight = createSubTextStyle({ name: 'lineHeight', cssName: 'line-height' });

const renderNumberToPx = (v: string) => (/^\d+$/.test(String(v)) ? `${v}px` : v);

export const FontSize = createSubTextStyle({ name: 'fontSize', cssName: 'font-size', renderValue: renderNumberToPx });
export const MarginLeft = createStyle({ name: 'marginLeft', cssName: 'margin-left', renderValue: renderNumberToPx });
export const MarginTop = createStyle({ name: 'marginTop', cssName: 'margin-top', renderValue: renderNumberToPx });
export const MarginRight = createStyle({ name: 'marginRight', cssName: 'margin-right', renderValue: renderNumberToPx });
...
function createSubTextStyle(o: CreateStyleOptions) {
	...
	return Extension.create<SubTextStylesOptions>({
		...
		addOptions() {
			return { types: ['textStyle'], defaultUnit: '' };
		},
					...
					attributes: {
						[name]: {
							...
							renderHTML: (attributes) => {
								...
								return { style: `${cssName}: ${renderValue(attr)}` };
							},
						},
					},
				},
