...
export type CSSColumnsOptions = { types: string[]; columnTypes: number[]; defaultColumnType: string };

declare module '@tiptap/core' {
	interface Commands<ReturnType> {
		cssColumns: { toggleColumns: (columnType: number | string) => ReturnType; unsetColumns: () => ReturnType };
	}
}
/**
...
export const CssColumnsExtension = Extension.create<CSSColumnsOptions>({
	name: 'cssColumns',
	addOptions() {
		return { types: [], columnTypes: [2, 3], defaultColumnType: 'two' };
	},
	addGlobalAttributes() {
		return [
			{
				...
				attributes: {
					cssColumns: {
						...
						renderHTML: (attributes) => {
							...
							return { style: `column-count: ${attributes['cssColumns']}` };
						},
